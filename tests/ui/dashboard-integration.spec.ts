...
test.describe('DASHBOARD INTEGRATION — SPEC-046', () => {
  ...
  test('[UI-229] Dashboard — KPI 显示真实任务数', async ({ page }) => {
    ...
    // Verify via API
    const statsRes = await page.request.get(`${API_BASE}/dashboard/stats`, {
  ...
  test('[UI-231] Dashboard — 任务状态分布准确', async ({ page }) => {
    ...
    if (statsRes.ok()) {
      const data = await statsRes.json();
      console.log('[UI-231] Task stats:', JSON.stringify(data.task_stats || data));
    }
  });
  ...
  test('[UI-235] Dashboard — 多用户数据隔离', async ({ page, request }) => {
    ...
    const statsB = await request.get(`${API_BASE}/dashboard/stats`, {
