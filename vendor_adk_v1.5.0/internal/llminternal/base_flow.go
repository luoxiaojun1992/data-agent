	...
	return func(yield func(*session.Event, error) bool) {
		for {
			...
			if lastEvent.LLMResponse.Partial {
				// The last event is partial (max token limit or stream ended without
				// finish_reason). This is not a fatal error — the response text has
				// already been delivered. Log and return gracefully.
				log.Printf("[adk-patch] last event is partial (may have hit max_tokens), text delivered: %d parts", len(lastEvent.Content.Parts))
				return
			}
		}
	}
...
func (f *Flow) runOneStep(ctx agent.InvocationContext) iter.Seq2[*session.Event, error] {
	return func(yield func(*session.Event, error) bool) {
		...
		for resp, err := range f.callLLM(ctx, req, stateDelta, artifactDelta) {
			...
			if resp.Content == nil && resp.ErrorCode == "" && !resp.Interrupted && resp.FinishReason == "" {
				continue
			}
