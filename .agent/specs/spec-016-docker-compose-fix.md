...
1. **移除 MinIO（含 etcd）**：根据架构设计（SPEC-003/SPEC-005），系统已全面使用 SeaweedFS 替代 MinIO 作为对象存储，且 Milvus v2.5.9 standalone 模式支持本地嵌入式存储，不再依赖外部 MinIO 和 etcd。
...
- 两个文件中 `milvus` 服务通过环境变量 `MINIO_ADDRESS=minio:9000` 和 `ETCD_ENDPOINTS=etcd:2379` 依赖外部存储
- 架构文档（`ARCHITECTURE.zh-CN.md`）明确对象存储为 **SeaweedFS**，技术栈列表中不包含 MinIO

### 2.2 应用服务缺失

- `docker-compose.yml` 仅包含基础设施服务（MongoDB, Redis, Milvus, etcd, Minio, SeaweedFS, Vault），缺少 `data-agent` 后端和 `frontend` 前端
- `docker-compose.ui-test.yml` 已包含 `data-agent` 服务和 `ui-e2e` 测试服务，但缺少 `frontend` 前端服务
- 开发人员无法通过 `docker compose up` 一键启动完整开发环境

### 2.3 Milvus standalone 模式

Milvus v2.5.9 的 `milvus run standalone` 默认使用本地嵌入式存储（Etcd + 本地磁盘），不需要外部 MinIO 和 etcd 服务。只需确保有持久化卷挂载即可。
...
| 删除 `minio:` 服务块 | ✅ | ✅ |
| 删除 `minio-data:` 卷 | ✅ | ✅ |
| milvus 环境变量移除 `MINIO_ADDRESS` | ✅ | ✅ |
| milvus 环境变量移除 `ETCD_ENDPOINTS` | ✅ | ✅ |
| milvus `depends_on` 移除 `etcd` | ✅ | ✅ |
| milvus `depends_on` 移除 `minio` | ✅ | ✅ |
| milvus 添加 `volumes: milvus-data:/var/lib/milvus` | ✅ | ✅ |
| 删除 `etcd:` 服务块 | ✅ | ✅ |
| 删除 `etcd-data:` 卷 | ✅ | ✅ |
| volumes 新增 `milvus-data:` | ✅ | ✅ |

> **注意**：etcd 和 minio 一并移除，因为 etcd 的唯一消费者是 Milvus，移除 Milvus 的外部 etcd 依赖后 etcd 不再被任何服务使用。
...
- 移除 milvus 对 minio/etcd 的依赖（同 4.1）
- 移除 minio/etcd 服务块和卷
- 添加 `frontend` 前端服务
...
| 是否需要新 DB 集合 | No |
| 是否影响现有 API | No |
| 性能影响 | 正面 — Milvus 嵌入式存储减少网络层，MinIO 移除降低启动时间 ~15s |
| 是否需要新增 Skill | No |
...
4. **Milvus test**: 验证 Milvus standalone 不依赖 minio/etcd 仍正常运行
...
- [ ] `docker compose up` 启动后，所有 7 个服务（mongodb, redis, milvus, seaweedfs, vault, data-agent, frontend）healthcheck 均为 healthy
- [ ] `minio` 服务和 `etcd` 服务不再存在于两个 compose 文件中
- [ ] `milvus` 不依赖外部 minio/etcd（无 `depends_on` 指向它们，无对应的环境变量）
