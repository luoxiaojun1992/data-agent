...
| 13.18 | SPEC-100 | 📐 优化 Token 用量埋点（session 独立统计表 + 删除级联 + session token 展示） | 立项（2026-09-16，方向已定）；**决策①** hourly record 不关联 session（stats_hourly 保持 {metric,hour} 全局不动）；**决策②** session token 独立建表 session_stats（_id=session_id，$inc 幂等累计），删 session 级联删除（Manager.HardDelete 先于主记录、DeleteOne 幂等）；**决策③** 不加时间冗余字段（UTC time.Time 绝对时间戳天然兼容时区，一年 ≤8760/指标实时聚合足够）；**澄清④** 级联仅硬删除、软删除（归档 Delete）不级联；**澄清⑤** 日历维度（小时/星期几/几号/几月）分桶按实际时区 Asia/Shanghai 计算——⚠️ 现状 bucketStart/bucketHours 全 .UTC() 分桶存在 8 小时口径偏差，本 spec 一并修正；**决策⑥** 分桶由后端处理 → 后端直接按 Asia/Shanghai 分桶（不改 API/不传时区），前端 dashboard 横轴同步按 Asia/Shanghai 格式化（分桶口径=展示口径）；**决策⑦** 子 session 用父 sessionID 计数（session_stats 统一父维度），删子 session 不级联；**决策⑧** 已实测确认 stats_hourly 无 session_id（代码+线上 197 条文档），无需删除；**决策⑨** 附带修正 get_current_time 按 UTC 返回+时区标注（归属 SPEC-080）；llmstats.Record 已带 SessionID 但 Incr 落库丢弃 → Recorder 双写 session_stats |
