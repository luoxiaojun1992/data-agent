...
| SPEC-100 | 优化 Token 用量埋点（session 独立统计表 + 删除级联 + session token 展示 + 时区口径修正） | **P15** | [spec-100-token-metrics-session-and-time-redundancy.md](spec-100-token-metrics-session-and-time-redundancy.md) | 📐 立项（方向已定） |
| SPEC-101 | Tool 用户批准执行开关（requires_approval DB 化，独立字段不入 config JSON） | **P15** | [spec-101-tool-approval-switch.md](spec-101-tool-approval-switch.md) | 📐 立项 |
...
| 13.18 | SPEC-100 | 📐 优化 Token 用量埋点（session 独立统计表 + 删除级联 + session token 展示） | 立项（2026-09-16，方向已定）；**决策①** hourly record 不关联 session（stats_hourly 保持 {metric,hour} 全局不动）；**决策②** session token 独立建表 session_stats（_id=session_id，$inc 幂等累计），删 session 级联删除（Manager.HardDelete 先于主记录、DeleteOne 幂等）；**决策③** 不加时间冗余字段（UTC time.Time 绝对时间戳天然兼容时区，一年 ≤8760/指标实时聚合足够）；**澄清④** 级联仅硬删除、软删除（归档 Delete）不级联；**澄清⑤** 日历维度（小时/星期几/几号/几月）分桶按实际时区 Asia/Shanghai 计算——⚠️ 现状 bucketStart/bucketHours 全 .UTC() 分桶存在 8 小时口径偏差，本 spec 一并修正；**决策⑥** 分桶与展示统一按**浏览器时区**——前端传 timezone 参数（Intl resolvedOptions().timeZone），后端 LoadLocation 校验（无效 400/缺省回退 Asia/Shanghai），按前端时区确定时间区间 → 转 UTC 区间查 DB → 按前端时区分桶聚合，横轴浏览器时区 toLocaleString 口径天然一致（⚠️ 需 import _ "time/tzdata"，容器无 tzdata 包；⚠️ 分桶/聚合 100% 后端执行，严禁原始 hourly 数据回前端聚合）；**决策⑦** 子 session 用父 sessionID 计数（session_stats 统一父维度），删子 session 不级联；**决策⑧** 已实测确认 stats_hourly 无 session_id（代码+线上 197 条文档），无需删除；**决策⑨** 附带修正 get_current_time 按 UTC 返回+时区标注（归属 SPEC-080）；llmstats.Record 已带 SessionID 但 Incr 落库丢弃 → Recorder 双写 session_stats |
| 13.19 | SPEC-101 | 📐 Tool 用户批准执行开关（requires_approval DB 化） | 立项（2026-09-17）；tool 增加「是否需要用户批准执行」开关，`skill_configs` 加独立字段 `requires_approval`（**不入 value config JSON**）；现状 hardcode 批准点 = file_delete/dir_delete 函数体内 `if deps.HumanGate != nil` Confirm → 迁移为读 DB 开关；一次性 mongosh 脚本补线上 32 条存量文档字段（file_delete/dir_delete=true，其余 false，幂等 $exists:false）；seed 同步 predefinedSkills()（仅这俩 true）；执行路径：开关 true → Confirm；**开关查询失败 → fail-closed 拒绝**；**HumanGate nil → 拒绝执行 + 错误日志（fail-closed，晓军拍板 2026-09-17，现有 backward-compat 测试需反转）**；ask_user 不纳入（HITL 提问语义，非批准开关） |
