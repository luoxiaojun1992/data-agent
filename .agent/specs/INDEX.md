...
| SPEC-069 | agent 调用子 agent（sub agent tool + interface 解耦、能力提示词单独组装、独立 session 父绑定返回即硬删、最终返回同 tool response、model 与主 agent 一致、并行委派） | **P15** | [spec-069-agent-invoke-subagent.md](spec-069-agent-invoke-subagent.md) | 📐 调研完成（方案已定，待实现） |
| SPEC-070 | 领域内聚重构（业务领域 logic/service/db_model 垂直切片，替换水平分层） | **P15** | [spec-070-domain-cohesion-refactor.md](spec-070-domain-cohesion-refactor.md) | 📐 立项（不展开） |
...
[P15] SPEC-068 ─── compaction 机制缺陷修复 + summary 语义拆分 + raw_events 存储重构
...
[P15] SPEC-069 ─── agent 调用子 agent (调研完成，方案已定，待实现)
