...
| 13.20 | SPEC-102 | ✅ 审计日志页面功能优化（搜索分页统一 + action 描述 + 参数校验 + 可见性） | **已实现并部署验证（2026-09-22，commit 61ecd7c）**；①前端搜索/分页样式对齐 SPEC-075/078（q + page/page_size 替换 skip/limit + 自定义下拉）；②全 DB 层筛选分页；③后端 hardcode mapping「METHOD FullPath 模板 → 中文描述」——**middleware 写入的 Action 已是 `method + c.FullPath()`（不含 query）正好作 key，无需新增字段**；展示输出 action_desc（未命中回退原始值），CSV 同步；④参数校验 400 化；⑤可见性过滤；**D1** q = 操作人 email 模糊搜索（SearchByEmail topN=10 → userIDs → `user_id $in`，现有 UserID 过滤语义平移）；**D2** path 参数 API 路径模糊搜索（匹配 action 字段 `$regex`+QuoteMeta，≤100）；**D3** status_class 枚举 1xx~5xx（非法 400，构造 `status_code: {$gte:X00,$lt:(X+1)00}`，前端下拉 audit-status-select）；**D4** mapping 清单按 routes.go CUD 路由创建，仅列表返回映射 action_desc（不作筛选）；**D5** 前端筛选栏匹配现有列表样式（q+path+状态码下拉+日期+筛选/重置；Pagination；导出弹窗收敛 csv）；**D6** 参数校验全清单（q/path≤100、status_class/format 枚举、page≥1、page_size 1~100、export limit 1~50000、日期格式+start≤end，违规一律 400 严禁静默容错）；**D7** audit_logs TTL 索引 `{created_at:1}` expire 365d（保留一年，幂等 EnsureIndexes）；**D8** 时间筛选不限制过去一年（start 不钳制自然空结果，前端不设 min）；**D9（2026-09-18）** 可见性——**RBAC 入口已满足无需改**（rbac_perm_audit_view 仅挂 rbac_role_admin L1 + system_admin L0 层级，普通用户 L2 403；fixed 角色为死代码）；行级过滤与 KB 同模式（system_admin 全量 / **admin 与 user 同级**看自己+无 user 关联 `$or [user_id=自己, 空, 不存在]`，判定用主角色 ctx["role"]）；普通用户分支逻辑保留（将来开放即生效）；导出同过滤；**调研发现的 bug**：前端操作类型下拉枚举与 DB Action 完全不匹配、选了查不到 |
| 13.21 | SPEC-103 | ✅ 纯前端国际化（i18n 翻译 + 语言切换） | **已实现并部署验证（2026-09-23，commit 87c525e）**；next-intl ^3.26.5（MIT）+ `lib/i18n/`（config/request/messages）40 命名空间 zh/en 字典 key 完全对齐；layout 接 NextIntlClientProvider + 防闪烁 inline script 对齐 `<html lang>`；LanguageToggle（cookie NEXT_LOCALE + localStorage 镜像 → router.refresh()，按钮固定显目标语言 EN/中文）置于主题切换后；40+ 页面硬编码中文抽离 t()（后端返回内容/代码块/testid/API 路径不翻译，D3/D5）；SSR cookie 驱动验证（zh 19 处「登录」/en 12 处 Email）；E2E 3/3 passed（默认中文/切换即时生效/cookie 持久化）；**测试基建增强**（playwright.config.ts）：PW_CHANNEL=chrome 复用系统 Chrome（本地 chromium-1228 下载不可行）+ PW_NO_PROXY=1 注入 --no-proxy-server（WorkBuddy 沙箱代理 HTTP_PROXY 劫持浏览器 API 请求致「服务离线」误报）+ API_BASE/E2E_USERNAME env 覆盖；**遗留债务**：存量 E2E 套件仍调 SPEC-084 已删除的 /auth/register（404），待专项迁移预置用户模式 |
