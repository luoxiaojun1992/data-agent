...
| 13.18 | SPEC-100 | ✅ 优化 Token 用量埋点（session 独立统计表 + 删除级联 + session token 展示） | **设计定稿（2026-09-17，D1~D7 全定稿，暂不实现）**；决策①~⑨（2026-09-16）：stats_hourly 不动 / session_stats 独立表（_id=session_id，$inc 幂等）/ HardDelete 先删统计幂等 / 软删不级联 / 无时间冗余字段 / **分桶按前端传 timezone（LoadLocation 校验，无效 400，缺省 Asia/Shanghai）后端分桶聚合（严禁原始数据回前端）** / 子 session 归父 / stats_hourly 无 session_id 已实测 / get_current_time 纯 UTC；**定稿细节**：D1 列表内嵌 token_tokens（sessions+task 列表，$in 批量查防 N+1）；D2 仅主键索引（不建 user_id/TTL）；D3 SessionStatStore 三方法（Upsert/DeleteBySession/GetBySessions，llmstats 同包）；D4 RecordingLLM 经 agent.InvocationContext 取 state["session_id"]——三场景天然统一（主 chat/task=自身、子 agent=父 session，subagent/tool.go:59），归父零成本；D5 Recorder 双写（SessionID 空仅全局）；D6 Manager.HardDelete 首行级联；D7 Series 加 loc 参数 + timezone 参数 + import _ "time/tzdata"（容器无 tzdata）+ get_current_time 5 字段全 UTC |
