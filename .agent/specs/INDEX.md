...
| SPEC-093 | Chat 输入框本地脱敏（OpenAI Privacy Filter / WebGPU 浏览器端推理；脱敏按钮 + 自动脱敏开关 localStorage 默认关闭；仅限输入框文本不含图片/PDF；模型未就绪强制禁用开关；运行时从 HF 下载加载 + 浏览器 Cache API 缓存） | **P15** | [spec-093-chat-input-local-redaction.md](spec-093-chat-input-local-redaction.md) | ✅ 已实现 |
...
| 13.11 | SPEC-093 | ✅ Chat 输入框本地脱敏（OpenAI Privacy Filter / WebGPU） | 已实现（2026-09-09）；纯前端（openai/privacy-filter Apache-2.0 + Transformers.js WebGPU→wasm q4f16）；脱敏按钮 + 自动脱敏开关（localStorage chat_auto_redact 默认关闭，ready 才读）；仅限输入框文本；脱敏中盾牌弹窗动画（双场景+≥300ms 防闪烁）；D7 修订：模型不进代码库，运行时从 HF 下载 + 浏览器 Cache API 缓存（useBrowserCache）；onnxruntime-node install 用 ONNXRUNTIME_NODE_INSTALL=skip（Nuget 302，浏览器端用 onnxruntime-web）；next.config.js alias 浏览器入口 + .mjs ESM 规则；UI-219~223 |
