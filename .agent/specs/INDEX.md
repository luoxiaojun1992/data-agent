...
| SPEC-097 | API Host 系统配置 + 公开查询接口（无 JWT、无 RBAC）+ 前端运行时动态获取（登录接口也走动态 host；拿不到 fallback 到当前前端 host） | **P15** | [spec-097-api-host-config-and-public-endpoint.md](spec-097-api-host-config-and-public-endpoint.md) | ✅ 设计定稿（D1~D4 全定稿；暂不实现） |
...
| 13.14 | SPEC-096 | ✅ Task PDF 解析 + kb/chat/task Excel 解析 | 设计定稿（2026-09-12，暂不实现）；D1 定稿 B（PDF 文字 `[PDF:name]` 标签拼进 description 存 DB，与 chat 同构）；D2 定稿 **task 域新增自己特有的校验常量/函数**（新建 `internal/domain/task/limits.go`，不复用 `domainchat`）：`MaxTaskTextBytes=100KB` + `MaxTaskImages=5`/`MaxTaskImageBytes=2MB`/`MaxTaskTotalBytes=5MB` + 特有 `ImagePart`/`ValidateTaskImages`/`ValidateTaskText`（XSS 规则仍复用 `security.ValidateXSS`，仅校验用户原文、PDF/Excel 文字豁免）；D3 定稿 `[Excel:name]` 标签；D4 定稿全部 sheet 拼接；D5 定稿 `read-excel-file`（MIT 走 package.json，**仅 `.xlsx` 不支持 `.xls`**）；D6 定稿 kb 复用 txt。**三端校验矩阵（§2.5）确认：task 文本长度+图片校验当前借用 chat，D2 定稿后改为 task 域自持** |
| 13.15 | SPEC-097 | ✅ API Host 系统配置 + 公开查询接口 + 前端动态获取 | 设计定稿（2026-09-11）；系统配置 `SystemBuiltins()` 新增 API_HOST 项；`GET /api/v1/api-host` **完全公开（无 JWT、无 RBAC）**，套用 SPEC-079 /health 范式；前端 `getApiHost()` 相对路径运行时获取、拿不到/空则 fallback `window.location.origin`；**登录接口也走动态 host**（API_BASE 从编译时常量改运行时解析，`await ensureApiHost()`）；D1 定稿 key=大驼峰 `API_HOST`；D2 定稿 api_host=**纯 origin（不含 /api/v1）**，`/api/v1` 由 getApiHost 唯一出口拼接、与 fallback 语义对齐；D3 定稿重构 API_BASE 为运行时动态；D4 定稿 origin fallback。**原始 seed 数据同步已确认（2026-09-12）：`SystemBuiltins()` 是唯一 seed 来源，`SeedBuiltins()` 逐 key 补缺（`wire.go:427` 启动调用），新增 API_HOST 重启自动 seed 进 system_configs、无需手动脚本（区别于 SPEC-098 skill seed 的整体跳过）**。**暂不实现** |
