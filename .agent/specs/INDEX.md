...
| SPEC-096 | Task PDF 解析（仅常规创建弹窗，日常总结模版不加）+ kb/chat/task 三端 Excel 解析（纯文本、无图片）；task 描述隐藏 PDF 内容只显示 📄；校验/显示/XSS 豁免与 PDF 一致 | **P15** | [spec-096-task-pdf-and-excel-parsing.md](spec-096-task-pdf-and-excel-parsing.md) | 📐 深化中（D1~D6 已定稿；D5 遗留 .xls 范围待确认；暂不实现） |
...
| 13.14 | SPEC-096 | 📐 Task PDF 解析 + kb/chat/task Excel 解析 | 深化（2026-09-12，暂不实现）；D1 定稿 B（PDF 文字 `[PDF:name]` 标签拼进 description 存 DB，与 chat 同构）；D2 定稿新增 `MaxTaskTextBytes=100KB`（校验整个 description=用户原文+PDF+Excel 合并；XSS 仅校验用户原文、PDF/Excel 文字豁免；图片含 PDF 配图复用 `ValidateImages`）；D3 定稿 `[Excel:name]` 标签；D4 定稿全部 sheet 拼接；D5 定稿 `read-excel-file`（MIT 走 package.json，⚠️ 仅 `.xlsx` 不支持 `.xls` 待确认）；D6 定稿 kb 复用 txt。**三端校验矩阵（§2.5）确认：task 仅缺文本长度，XSS+图片已复用 chat** |
