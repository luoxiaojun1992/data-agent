...
| SPEC-094 | 前端 XSS 输出转义组件 + 后端输入限制校验补齐（KB/chat/task 文本长度与图片数量/大小后端兜底；XSS 不做后端，统一由前端输出转义承担） | **P15** | [spec-094-frontend-xss-output-escape-and-backend-input-limits.md](spec-094-frontend-xss-output-escape-and-backend-input-limits.md) | 📐 立项（暂不深化） |
...
| 13.12 | SPEC-094 | 📐 前端 XSS 输出转义 + 后端输入限制补齐 | 立项（暂不深化，定稿后实现）；前端 SafeText 转义组件（输出转义）；后端只补 KB/chat/task 文本长度与图片数量/大小兜底，XSS 不做；现状已查：KB（200 runes/5MB/10 图/1MB）✅、chat（100KB/5 图/2MB/总量 5MB）✅、task 长度与 chat PDF 大小 ❓ 待查；待调查 5 项清单已列 |
