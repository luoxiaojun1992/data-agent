...
| 13.11 | SPEC-093 | 📐 Chat 输入框本地脱敏（OpenAI Privacy Filter / WebGPU） | 独立可插队；纯前端（openai/privacy-filter Apache-2.0 1.5B/50M MoE + Transformers.js WebGPU q4）；脱敏按钮 + 自动脱敏开关（localStorage 默认关闭）；仅限输入框文本（不动图片/PDF 附件）；模型未就绪强制禁用开关（ready 后才读 localStorage）；模型文件预置 public/models 零外网；D1~D7 决策点待拍板 |
