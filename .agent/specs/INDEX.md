...
| 13.19 | SPEC-101 | 📐 Tool 用户批准执行开关（requires_approval DB 化） | 立项（2026-09-17）；tool 增加「是否需要用户批准执行」开关，`skill_configs` 加独立字段 `requires_approval`（**不入 value config JSON**）；现状 hardcode 批准点 = file_delete/dir_delete 函数体内 `if deps.HumanGate != nil` Confirm → 迁移为读 DB 开关；一次性 mongosh 脚本补线上 32 条存量文档字段（file_delete/dir_delete=true，其余 false，幂等 $exists:false）；seed 同步 predefinedSkills()（仅这俩 true）；执行路径：开关 true → Confirm；**开关查询失败 → fail-closed 拒绝**；HumanGate nil → 保持现状直接执行（生产 wire.go 恒注入、主/子 agent 共用 toolDeps，nil 仅单测兼容分支）；ask_user 不纳入（HITL 提问语义，非批准开关） |
