...
| SPEC-102 | 审计日志页面功能优化（搜索分页统一 + action 描述 mapping + 参数校验 + 可见性过滤） | **P15** | [spec-102-audit-log-page-optimization.md](spec-102-audit-log-page-optimization.md) | 📐 立项 |
...
| 13.20 | SPEC-102 | 📐 审计日志页面功能优化（搜索分页统一 + action 描述 + 参数校验 + 可见性） | 立项（2026-09-17）；①前端搜索/分页样式对齐 SPEC-075/078（q + page/page_size 替换 skip/limit + 自定义下拉）；②全 DB 层筛选分页；③后端 hardcode mapping「METHOD FullPath 模板 → 中文描述」——**middleware 写入的 Action 已是 `method + c.FullPath()`（不含 query）正好作 key，无需新增字段**；展示输出 action_desc（未命中回退原始值），CSV 同步；④参数校验 400 化（分页非法/日期格式错误现为 500/静默容错；export format 仅 csv）；⑤可见性过滤：非 system_admin（ctx role）→ `$or [user_id=自己, 空, 不存在]`，system_admin 全量，列表与导出一致；**D1 定稿（2026-09-17 晓军拍板）**：q = 操作人 email 模糊搜索——`SearchByEmail(q, topN=10)` → userIDs → 审计日志 `user_id $in`（现有 UserID 过滤语义平移，q 不做 action/details 正则）；**D2 定稿**：新增 `path` 参数 API 路径模糊搜索（匹配 `action` 字段 = "METHOD FullPath模板" 不含 query，`$regex`+QuoteMeta 忽略大小写，≤100 超长 400）；**D3 定稿**：新增 `status_class` 参数（枚举 1xx/2xx/3xx/4xx/5xx，非法 400，构造 `status_code: {$gte:X00,$lt:(X+1)00}` 范围，前端下拉 data-testid=audit-status-select）；**D4 定稿**：mapping 清单按 routes.go CUD 路由创建，**仅用于列表返回映射 action_desc**（不作筛选用途）；**D5 定稿**：前端筛选栏按条件适当设计匹配现有列表样式（q+path+状态码下拉+日期+筛选/重置，Pagination，导出弹窗收敛 csv）；**D6 定稿**：参数校验常规方案全清单（q/path 长度≤100、status_class/format 枚举、page≥1、page_size 1~100、export limit 1~50000、日期格式+start≤end，违规一律 400 严禁静默容错）；**D7 定稿**：audit_logs 建 TTL 索引 `{created_at:1}` expire 365 天（保留一年，幂等 EnsureIndexes，与 stats_hourly 口径一致）；**D8 定稿**：时间筛选不限制过去一年（start 不钳制，任意早日期自然空结果，前端不设 min）；全部过滤条件 AND 叠加；**调研发现的 bug**：前端操作类型下拉枚举（chat:query 等）与 DB Action（"POST /api/v1/chat"）完全不匹配、选了查不到 |
