...
| 13.18 | SPEC-100 | ✅ 优化 Token 用量埋点（session 独立统计表 + 删除级联 + session token 展示） | **设计定稿（2026-09-17，D1~D7 全定稿，暂不实现）**；决策①~⑨（2026-09-16）：stats_hourly 不动 / session_stats 独立表（_id=session_id，$inc 幂等）**不存 user_id（晓军确认冗余）** / HardDelete 先删统计幂等 / 软删不级联 / 无时间冗余字段 / **分桶按前端传 timezone（LoadLocation 校验，无效 400，缺省 Asia/Shanghai）后端分桶聚合（严禁原始数据回前端）** / 子 session 归父 / stats_hourly 无 session_id 已实测 / get_current_time 纯 UTC；**定稿细节（2026-09-17 晓军二次拍板）**：D1 **不做列表内嵌**——新增 `GET /api/v1/sessions/:id/token-usage` 点查（verifyOwnership）+ task run 详情（GetRun）内嵌 token_tokens；展示位置：chat 页消息历史下方/输入框按钮行上方（chat-token-usage，进入+流结束刷新）+ task run 详情区；D2 仅主键索引；D3 SessionStatStore 三方法（**Incr**/DeleteBySession/GetBySession，llmstats 同包，无 user_id；命名 Incr 对齐 Counter 增量语义，内部 $inc+upsert 实现）；D4 RecordingLLM 经 agent.InvocationContext 取 state["session_id"]——三场景天然统一（主 chat/task=自身、子 agent=父 session，subagent/tool.go:59），归父零成本；D5 Recorder 双写（不取 UserID）；D6 Manager.HardDelete **首行、主数据删除前**级联 DeleteBySession——**deletedCount=0（已不存在）视为成功继续，仅 err != nil 中止且主记录不删**；D7 Series 加 loc 参数 + timezone 参数 + import _ "time/tzdata"（容器无 tzdata）+ get_current_time 5 字段全 UTC |
| 13.19 | SPEC-101 | ✅ Tool 用户批准执行开关（requires_approval DB 化） | **设计定稿（2026-09-17，D1~D5 全定稿，暂不实现）**；`skill_configs` 加独立字段 `requires_approval`（不入 value config JSON）；file_delete/dir_delete 硬编码 Confirm → 读 DB 开关；一次性 mongosh 幂等脚本补线上 32 条（file_delete/dir_delete=true 其余 false）；seed 同步（仅这俩 true）；**执行路径（fail-closed 晓军拍板）**：开关 false 直接执行 / 开关 true → Confirm / 开关查询失败或 HumanGate nil → 拒绝 + 错误日志（现有 backward-compat 测试需反转）；ask_user 不纳入；**定稿细节**：D1 domain/repo/读取路径透传；D2 **复用现有 `Deps.SkillConfig`（已注入），不新增 Deps 字段不改 wire**，ConfigService 加 `RequiresApproval(ctx, name) (bool, error)`；D3 admin Upsert 请求体显式加字段；D4 seed+脚本；D5 前端 admin/skills 编辑弹窗加开关（data-testid=skill-approval-toggle）+ 🔒 标记，E2E 同步 |
