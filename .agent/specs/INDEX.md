...
| 13.16 | SPEC-098 | 📝 pptx skill 优化 — 同时支持 Markdown 与 HTML | 立项（2026-09-12，暂不实现）；根因：`pptx_generator` 仅用 `genppt.FromMarkdownWithOptions`，markdown 转换器只支持 text/bullet/code/heading/image 五种块、无表格无逐元素样式 → 丑；genppt 库**已内置** `FromHTMLWithOptions`（支持 `<table>`/`<img>` 圆角浮动/逐元素 color/font-size/align/background-color/幻灯片背景），本次不升级依赖只暴露它。变更三处：`logic/pptx` 新增 `GenerateHTML` + `adk/tools` 加 `format` 参数（markdown/html 默认 markdown）+ `service/skill` predefinedSkills 同步描述。**待定决策 D1（format 显式 vs sniff）/D2（HTML 默认配色）/D3（SeedSkills 幂等不更新已存在记录，线上描述需一次性手动同步）** |
