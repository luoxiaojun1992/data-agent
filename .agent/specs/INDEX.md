...
| SPEC-093 | Chat 输入框本地脱敏（OpenAI Privacy Filter / WebGPU 浏览器端推理；脱敏按钮 + 自动脱敏开关 localStorage 默认关闭；仅限输入框文本不含图片/PDF；模型未就绪强制禁用开关；模型文件预置前端资源目录零外网加载） | **P15** | [spec-093-chat-input-local-redaction.md](spec-093-chat-input-local-redaction.md) | 📐 设计已定稿 |
...
| 13.11 | SPEC-093 | 📐 Chat 输入框本地脱敏（OpenAI Privacy Filter / WebGPU） | 独立可插队；纯前端（openai/privacy-filter Apache-2.0 1.5B/50M MoE + Transformers.js WebGPU q4）；脱敏按钮 + 自动脱敏开关（localStorage 默认关闭）；仅限输入框文本（不动图片/PDF 附件）；模型未就绪强制禁用开关（ready 后才读 localStorage）；模型文件直提交代码库 public/models 零外网；脱敏中弹窗动画（盾牌 + 遮罩 + ≥300ms 防闪烁）；D1~D7 已拍板（占位符 `<PRIVATE_*>` 尖括号同 Presidio / wasm 降级 / 失败报错中止发送 / hover 联系管理员） |
