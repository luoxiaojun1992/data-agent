...
| SPEC-070 | KB 切片图数据库索引（Cayley）+ 图访问共用组件 + 图谱搜索 Skill（GraphRepository 接口 + knowledge_graph_search tool + seed） | **P15** | [spec-070-kb-graph-index.md](spec-070-kb-graph-index.md) | 📐 详细设计 |
...
[P15] SPEC-070 ─── KB 切片图数据库索引 (Cayley) + 图访问共用组件 + 图谱搜索 Skill (详细设计)
                   (GraphRepository 接口 + cayley infra 适配器，KB 与 Skill 共用;
                    仅 Chunk 节点 + RELATED_TO 边(最小图模型);
                    Cayley quad 模型存储 + bbolt 后端嵌入进程(无独立服务);
                    AddChunks 复用向量检索找同 creator 切片，topN=5 写图;
                    Chunk 含 creator_id/is_public，查询按 system_admin 策略过滤;
                    seed 仅 skill 配置(Cayley 无 schema，无存量回填);
                    文档删除三处级联清理(Mongo+Qdrant DeletePoints+Cayley 删 quad);
                    Cayley+bbolt 均 Apache-2.0/MIT(无 copyleft 顾虑);
