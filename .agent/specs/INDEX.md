...
| SPEC-090 | Session 生命周期管理（归档软删除 / 硬删除+workspace+chat history / 清空聊天历史 / 永久产物 artifact+memory 隔离；补归属校验防 IDOR） | **P15** | [spec-090-session-lifecycle-management.md](spec-090-session-lifecycle-management.md) | ✅ 已实现 |
...
| SPEC-096 | Task PDF 解析（仅常规创建弹窗，日常总结模版不加）+ kb/chat/task 三端 Excel 解析（纯文本、无图片）；task 描述隐藏 PDF 内容只显示 📄；校验/显示/XSS 豁免与 PDF 一致 | **P15** | [spec-096-task-pdf-and-excel-parsing.md](spec-096-task-pdf-and-excel-parsing.md) | ✅ 已实现（2026-09-20 实现并部署验证） |
...
| 13.8 | SPEC-090 | ✅ Session 生命周期管理（归档/删除/清空历史/永久产物隔离） | 已实现（2026-09-08）；DELETE 归档=纯软删保留 workspace+history（无 TTL）；DELETE?permanent=true 硬删除级联 sessions/adk_sessions/session_events/workspace+子 session（不删 artifact/memory）；DELETE /:id/history 清空历史；ListDeleted DB 层按 user 过滤；Cleanup 豁免归档 + 级联清理；Get/Delete/Restore/ClearHistory 补归属校验（403 / system_admin 豁免） |
...
| 13.13 | SPEC-095 | ✅ Chat 语音输入（whisper.wasm 纯 CPU 本地转写） | **已实现并部署验证（2026-09-15，commit 7e8816c）**；纯前端 + nginx 改动，后端无变化。**D5 触发 fallback**：npm 上 whisper.cpp 官方包是 Node-only（浏览器不可用），改用 Emscripten 6.0.9 自编译官方 `examples/whisper.wasm` 产物（`-DWHISPER_WASM_SINGLE_FILE=ON -s USE_PTHREADS=1 -s PTHREAD_POOL_SIZE_STRICT=0`）单文件 `libmain.js`（1.8MB）提交 `public/whisper/`；前端自写 `lib/voice.ts`（Module.init/full_default/FS_createDataFile + MediaRecorder + 16kHz 重采样 + `total time` 完成标志）。模型用多语言 `ggml-tiny.bin`（74MB，非 tiny.en，中文识别）提交 `public/models/whisper/`。**新增 nginx COOP/COEP 头**（same-origin + require-corp）：`full_default` 用 std::thread 跑转写、强制 SharedArrayBuffer。部署验证：COOP/COEP 头 ✅ + crossOriginIsolated=true ✅ + whisper 模块/FS_createDataFile 导出 ✅ + 模型加载 ✅ + 错误处理（headless 无麦克风 NotSupportedError 回 idle）✅ |
| 13.14 | SPEC-096 | ✅ Task PDF 解析 + kb/chat/task Excel 解析 | **已实现并部署验证（2026-09-20，commit 428c0dd）**；后端 task 域新增 `limits.go`（MaxTaskTextBytes=100KB / MaxTaskImages=5 / MaxTaskImageBytes=2MB / MaxTaskTotalBytes=5MB + ImagePart + ValidateTaskImages/ValidateTaskText，去 domainchat 依赖）；chat 域新增 `format.go`（formatPDFText/formatExcelText 标签）；ChatRequest 加 `excels`；task handler 加 `pdfs`/`excels` 字段 + `prependPDFExcelText` 拼 description（100KB 上限）；前端新增 `lib/excel.ts`（read-excel-file@9.3.10，MIT，仅 .xlsx）+ chat/agent/knowledge 三页接入（常规弹窗 PDF/Excel 附件、日常总结模版无附件入口）；task/run 详情页剥离 PDF/Excel 标签块。D1 定稿 B（PDF 文字 `[PDF:name]` 标签拼进 description 存 DB，与 chat 同构）；D2 定稿 **task 域新增自己特有的校验常量/函数**（新建 `internal/domain/task/limits.go`，不复用 `domainchat`）：`MaxTaskTextBytes=100KB` + `MaxTaskImages=5`/`MaxTaskImageBytes=2MB`/`MaxTaskTotalBytes=5MB` + 特有 `ImagePart`/`ValidateTaskImages`/`ValidateTaskText`（XSS 规则仍复用 `security.ValidateXSS`，仅校验用户原文、PDF/Excel 文字豁免）；D3 定稿 `[Excel:name]` 标签；D4 定稿全部 sheet 拼接；D5 定稿 `read-excel-file`（MIT 走 package.json，**仅 `.xlsx` 不支持 `.xls`**）；D6 定稿 kb 复用 txt。**三端校验矩阵（§2.5）确认：task 文本长度+图片校验当前借用 chat，D2 定稿后改为 task 域自持** |
| 13.15 | SPEC-097 | ✅ API Host 系统配置 + 公开查询接口 + 前端动态获取 | **已实现并部署验证（2026-09-18，commit 0a4b848）**；`SystemBuiltins()` 新增 API_HOST（纯 origin 不含 /api/v1，SeedBuiltins 逐 key 补缺重启自动 seed）；`GET /api/v1/api-host` 完全公开（无 JWT 无 RBAC，登录前可用）；前端新增 `lib/api-host.ts` getApiHost()（相对路径查询 + 缓存去重 + /api/v1 唯一拼接出口 + window.location.origin fallback）；lib/api.ts 的 API_BASE 编译时常量 → 运行时动态（login/apiFetch 均 await getApiHost）；**实际收敛 5 个文件**（chat/page.tsx 4 处 + users/ModelSelector/OnlineIndicator/register 各 1 处，spec 文档只列了 chat）；验证 7 项全过（接口空串/配置值/恢复空、seed、前端产物、公网无 JWT、登录闭环）；⚠️ 实测发现：直接 mongosh 改 DB 缓存不失效（SPEC-061 Cache-Aside），恢复配置必须走 admin API |
| 13.16 | SPEC-098 | ✅ pptx skill 优化 — 同时支持 Markdown 与 HTML | 设计定稿（2026-09-12，暂不实现）；根因：`pptx_generator` 仅用 `genppt.FromMarkdownWithOptions`，markdown 转换器只支持 text/bullet/code/heading/image 五种块、无表格无逐元素样式 → 丑；genppt 库**已内置** `FromHTMLWithOptions`（支持 `<table>`/`<img>` 圆角浮动/逐元素 color/font-size/align/background-color/幻灯片背景），本次不升级依赖只暴露它。变更三处：`logic/pptx` 新增 `GenerateHTML` + `adk/tools` 加 `format` 参数（markdown/html 默认 markdown）+ `service/skill` predefinedSkills 同步描述。**D1 定稿显式 `format` 参数（markdown/html 默认 markdown）；D2 定稿沿用深色系配色（#1E3A5F/#333/#FFFFFF，LLM 可 `<body style>` 覆盖背景）；D3 定稿更新原始 seed + 实现时一次性脚本直改线上 DB 描述（绕过 SeedSkills 幂等跳过，不改 SeedSkills 本身）** |
| 13.17 | SPEC-099 | ✅ 后端语音转写服务（whisper tiny 纯 CPU，分片上传 + 结束一次性转写） | 已实现并部署验证（2026-09-16）；替代 SPEC-095 纯前端 wasm 方案（纯 HTTP 非 localhost 下 COI 门槛致 wasm 挂死）；自研 whisper sidecar + 主后端 start/chunk/finish 分片协议；RBAC=PermChatView；转写原样返回不脱敏不增强 |
| 13.18 | SPEC-100 | ✅ 优化 Token 用量埋点（session 独立统计表 + 删除级联 + session token 展示） | **已实现并部署验证（2026-09-21，commit afeb32c）**；`session_stats` 独立集合（_id=session_id，无 user_id/无 TTL，仅主键索引）+ `SessionStatStore`（Incr/DeleteBySession/GetBySession，$inc+upsert 幂等）；Recorder 双写（全局 stats_hourly + 会话维度）；RecordingLLM 经 agent.InvocationContext 取 state["session_id"]（主 chat/task=自身、子 agent=父 session）；`GET /api/v1/sessions/:id/token-usage` 点查（PermChatView + verifyOwnership）+ GetRun 内嵌 token_tokens（读时注入不落库）；Manager.HardDelete 首行级联 DeleteBySession（deletedCount=0 视为成功、仅 err≠nil 中止）；Dashboard Series/bucketStart/bucketHours 带 loc 按 timezone 分桶（非法 400、缺省 Asia/Shanghai）+ import _ "time/tzdata"（容器无 tzdata）；get_current_time 全 UTC；前端 chat 页 token 显示（chat-token-usage，进入+流结束刷新）+ task run 详情 + dashboard 传 timezone；顺带修复 pre-existing stale test（SPEC-084 数据隔离断言，commit dac7039） |
| 13.19 | SPEC-101 | ✅ Tool 用户批准执行开关（requires_approval DB 化） | **已实现并部署验证（2026-09-21，commit 8509eca）**；`skill_configs` 加独立字段 `requires_approval`（不入 value config JSON）；file_delete/dir_delete 硬编码 Confirm → 读 DB 开关；一次性 mongosh 幂等脚本补线上 32 条（file_delete/dir_delete=true 其余 false）；seed 同步（仅这俩 true）；**执行路径（fail-closed 晓军拍板）**：开关 false 直接执行 / 开关 true → Confirm / 开关查询失败或 HumanGate nil → 拒绝 + 错误日志（现有 backward-compat 测试需反转）；ask_user 不纳入；**实现细节**：D1 domain/repo/读取路径透传（`SkillConfig.RequiresApproval` + `mongo.skillConfigDoc.RequiresApproval` + `Upsert $set` + converter）；D2 **复用现有 `Deps.SkillConfig`（已注入），不新增 Deps 字段不改 wire**，`ConfigService.RequiresApproval(ctx, name) (bool, error)`；D3 admin Upsert 请求体显式加字段；D4 seed+脚本（`predefinedSkills` file_delete/dir_delete=true）；D5 前端 admin/skills 编辑弹窗加开关（data-testid=skill-approval-toggle）+ 🔒 标记；**测试**：tools.go fail-closed 全分支（nil gate 拒绝 / lookup error 拒绝 / SkillConfig nil 拒绝 / approval false 直跑）+ service/mongo 单测；**E2E 验证**：admin/skills 列表与详情暴露 requires_approval、PUT 往返、前端产物含 skill-approval-toggle、迁移幂等（32 条已带字段 matched=0） |
| 13.20 | SPEC-102 | ✅ 审计日志页面功能优化（搜索分页统一 + action 描述 + 参数校验 + 可见性） | **设计定稿（2026-09-18，D1~D9 全定稿，暂不实现）**；①前端搜索/分页样式对齐 SPEC-075/078（q + page/page_size 替换 skip/limit + 自定义下拉）；②全 DB 层筛选分页；③后端 hardcode mapping「METHOD FullPath 模板 → 中文描述」——**middleware 写入的 Action 已是 `method + c.FullPath()`（不含 query）正好作 key，无需新增字段**；展示输出 action_desc（未命中回退原始值），CSV 同步；④参数校验 400 化；⑤可见性过滤；**D1** q = 操作人 email 模糊搜索（SearchByEmail topN=10 → userIDs → `user_id $in`，现有 UserID 过滤语义平移）；**D2** path 参数 API 路径模糊搜索（匹配 action 字段 `$regex`+QuoteMeta，≤100）；**D3** status_class 枚举 1xx~5xx（非法 400，构造 `status_code: {$gte:X00,$lt:(X+1)00}`，前端下拉 audit-status-select）；**D4** mapping 清单按 routes.go CUD 路由创建，仅列表返回映射 action_desc（不作筛选）；**D5** 前端筛选栏匹配现有列表样式（q+path+状态码下拉+日期+筛选/重置；Pagination；导出弹窗收敛 csv）；**D6** 参数校验全清单（q/path≤100、status_class/format 枚举、page≥1、page_size 1~100、export limit 1~50000、日期格式+start≤end，违规一律 400 严禁静默容错）；**D7** audit_logs TTL 索引 `{created_at:1}` expire 365d（保留一年，幂等 EnsureIndexes）；**D8** 时间筛选不限制过去一年（start 不钳制自然空结果，前端不设 min）；**D9（2026-09-18）** 可见性——**RBAC 入口已满足无需改**（rbac_perm_audit_view 仅挂 rbac_role_admin L1 + system_admin L0 层级，普通用户 L2 403；fixed 角色为死代码）；行级过滤与 KB 同模式（system_admin 全量 / **admin 与 user 同级**看自己+无 user 关联 `$or [user_id=自己, 空, 不存在]`，判定用主角色 ctx["role"]）；普通用户分支逻辑保留（将来开放即生效）；导出同过滤；**调研发现的 bug**：前端操作类型下拉枚举与 DB Action 完全不匹配、选了查不到 |
| 13.21 | SPEC-103 | ✅ 纯前端国际化（i18n 翻译 + 语言切换） | **设计定稿（2026-09-18，D1~D5 全定稿，暂不实现）**；纯前端 UI 文案翻译 + 语言切换，参考 SPEC-076 主题切换范式；**D1** 技术方案 **next-intl**（App Router 最成熟、SSR 友好、TS 类型安全、MIT ✅）；**D2** zh（默认）+en；**D3** **后端返回的一切不翻译**（LLM/记忆/KB/artifact/task/配置/审计 action_desc/通知/错误消息原样显示），只翻译前端静态 UI 文案；**D4** 语言存 **cookie**（SSR 可读，避免首屏闪中文变英文 + hydration mismatch，SPEC-076 同坑）+ localStorage 镜像 + inline script 防闪烁；**D4 副作用确认**：cookie ≈2 字节/同域 SameSite=Lax/后端零影响（不读）/无额外包（next-intl 标准 + Next.js middleware）/清 cookie 重置默认 zh 可接受/无迁移成本；**D4 附注**：主题（SPEC-076）与自动脱敏（SPEC-093）**不迁移 cookie**（主题是 data-theme 属性切换、inline script 已够；脱敏是客户端状态与 SSR 文本无关）；脱敏依赖 API 健康度在浏览器前端处理（同 isVoiceInputSupported 模式）；**D5** **一次性全站翻译不分期**，只翻译纯文字（不碰 HTML/JS/代码块/markdown/类名/testid/API 路径），不影响样式结构显示效果，**中英长度差异自适应**（不写死宽度/word-break/flex），未翻译 key 降级回退 zh |
