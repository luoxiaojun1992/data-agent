...
| 13.18 | SPEC-100 | 📐 优化 Token 用量埋点（sessionID + 时间冗余字段 + 索引 + 趋势聚合优化 + session token 展示） | 立项（2026-09-16，暂不展开调研深化）；stats_hourly 加 session_id + 时间冗余字段（hour_of_day/day_of_week/day_of_month/month）+ 索引；llmstats.Record 已带 SessionID 但 Incr 落库丢弃 → Counter 接口扩展；dashboard 趋势聚合下沉 MongoDB；chat/task 页展示 session token |
