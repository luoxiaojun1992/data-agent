...
| 13.19 | SPEC-101 | 📐 Tool 用户批准执行开关（requires_approval DB 化） | 立项（2026-09-17）；tool 增加「是否需要用户批准执行」开关，`skill_configs` 加独立字段 `requires_approval`（**不入 value config JSON**）；现状 hardcode 批准点 = file_delete/dir_delete 函数体内 `if deps.HumanGate != nil` Confirm → 迁移为读 DB 开关；一次性 mongosh 脚本补线上 32 条存量文档字段（file_delete/dir_delete=true，其余 false，幂等 $exists:false）；seed 同步 predefinedSkills()（仅这俩 true）；执行路径：开关 true → Confirm，HumanGate nil 或开关查询失败 → fail-closed 拒绝（待拍板）；ask_user 不纳入（HITL 提问语义，非批准开关） |
