...
| SPEC-103 | 纯前端国际化（i18n 翻译 + 语言切换） | **P15** | [spec-103-frontend-i18n-language-switch.md](spec-103-frontend-i18n-language-switch.md) | 📐 立项（暂不深化） |
...
| 13.21 | SPEC-103 | 📐 纯前端国际化（i18n 翻译 + 语言切换） | 立项（2026-09-18，暂不深化）；纯前端 UI 文案翻译化 + 语言切换（默认中文、localStorage 持久化、即时生效），参考 SPEC-076 主题切换范式；不翻译 LLM 对话/用户内容；**待定决策点**：①i18n 技术方案（next-intl/i18next/自研轻量）②语言范围（推测 zh+en 起步）③**后端返回中文文案处理**（审计 action 描述/通知/错误消息——纯前端无法翻译，需定边界）④App Router SSR 水合一致性 ⑤翻译覆盖范围分批 |
