...
> **SPEC-007** | Status: 设计中 | 依赖: SPEC-004（Skill 接口/注册中心）, **SPEC-015（数据分析 Logic 层）**, SPEC-005（知识库基础设施）, SPEC-006（Artifact 存储 + 工作区管理）

## 目标

实现所有 **Skill 包装层**的具体代码。每个 Skill 只做：参数校验 → 权限检查 → 调用 Logic 层 / 基础设施 → 格式化输出。

> 数据分析核心 Logic（SQL/Stats/Knowledge/Report 校验）见 **SPEC-015**，必须先于本 spec 完成。
...
| SPEC-015 | ✅/❌ | 数据分析 Logic 层可用 |
| SPEC-005 | ✅/❌ | GridFS + Milvus Collection 可用（knowledge_search 依赖） |
| SPEC-006 | ✅/❌ | Artifact 存储 + 工作区管理可用（save_artifact / workspace_* 依赖） |

## 背景

SPEC-004 定义了 Skill 接口和注册机制，SPEC-015 实现了数据分析核心 Logic，本 spec 负责把 Logic 封装成 Agent 可调用的 Skill。Skill 层代码应足够薄：不包含业务算法，只处理输入输出适配。
...
- 调用 Logic：`openapi.Parse()` → `openapi.ToMCPTools()`（见 SPEC-015）
- 双重审核：管理员审批 + 域名白名单 → 上线
...
| `internal/logic/` | 共用 Logic 层（见 SPEC-015） | 引用 |

## 验证标准

1. `sql_executor` 注入 `DROP TABLE` → 拒绝（调用 SPEC-015 Logic 层校验）
