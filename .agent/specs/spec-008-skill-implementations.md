...
```
OpenAPI 3.0 Spec → 解析生成 MCP Tool 定义 → 管理员审批 → 注册到 Skill Registry
                                                              │
                                              Agent 调用 Execute()（Go 函数调用）
                                                              │
                                              Skill 内部构造 HTTP 请求 → 调用外部 API → 返回
```

- **参数校验**：`spec_url` 必填，检查 OpenAPI 版本 ≥ 3.0
- **OpenAPI 解析**（Skill 自含）：
  - 解析 JSON/YAML 格式的 OpenAPI 3.0 规范
  - 提取 `paths` / `parameters` / `requestBody` / `responses` Schema
  - 自动生成 MCP Tool 定义（name / description / inputSchema）
- **双重审核**：管理员审批 + 域名白名单 → 上线
- **运行时执行**：
  - Agent → `skill.Execute()` 直接调用（无 HTTP，Go 函数调用）
  - Skill 内部构造 HTTP 请求 → 调用外部 API
  - 调用频率限制（QPS 可配置）
  - 审计日志记录每次外部 API 调用
- **DB 集合**：`openapi_tools`（元数据 + 审批状态）
