...
- **触发时机（两种场景都必须展示弹窗动画）**：
  1. **手动点击「脱敏」按钮**：按钮点击 → 弹窗动画展示 → 推理结束回填输入框 → 弹窗关闭；
  2. **提交时自动脱敏**（开关开启）：用户点击发送 → **先弹窗动画**（覆盖推理等待期）→ 推理完成 → 弹窗关闭 → 再进入正常发送流程（构造 userMsg / fetch SSE / streaming=true）。弹窗期间发送流程**不启动**（streaming 保持 false），推理失败则弹窗关闭 + 报错 + 中止发送（不发原文）。
...
2. **E2E tests**（`tests/ui/chat-redact.spec.ts`，编号 `UI-XXX`）：
   ...
   - **脱敏中弹窗**：手动点击推理期间 `chat-redact-overlay` 可见（盾牌动画元素在）；**自动脱敏提交时弹窗同样展示**（发送前、streaming 未启动）；推理结束弹窗关闭；最小展示时长防闪烁（mock 慢推理/快推理两分支）；
...
13. **脱敏中弹窗（两种场景都必须有）**：① 手动点击脱敏 → 推理期间遮罩 + 盾牌动画可见（`chat-redact-overlay`/`chat-redact-shield`），结束自动关闭；② 自动脱敏提交 → 发送前弹窗动画展示（streaming 未启动），推理完成弹窗关闭后才进入发送流程；快推理（<300ms）弹窗仍可见 ≥300ms 防闪烁；推理失败弹窗关闭 + 错误提示；脱敏中按钮 disabled 不可重复触发、点击遮罩不关闭。
