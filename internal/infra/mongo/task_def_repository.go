...
func (r *TaskDefRepository) Delete(ctx context.Context, id string) error {
...
func (r *TaskRunRepository) Cancel(ctx context.Context, id string) error {
	_, err := r.coll.UpdateOne(ctx,
		bson.M{"_id": id, "status": bson.M{"$in": []string{string(task.StatusQueued), string(task.StatusPending), string(task.StatusRunning)}}},
