...
| Phase 2 | W3-4 | SPEC-004, SPEC-006, SPEC-015 | Agent 核心引擎（含安全审计）+ Artifact + 数据分析 Logic 基础 |
| Phase 3 | W5-6 | SPEC-005, SPEC-015 | 知识库系统 + 数据分析 Logic 完成 |
| Phase 2～4 | — | SPEC-007 | Skill 实现层（跨阶段实现） |
| Phase 2～4 | — | SPEC-014 | 任务队列 / Worker / 调度器（跨阶段实现） |
| Phase 4 | W7-8 | SPEC-009, SPEC-010, SPEC-014（调度器） | 统计监控 + 飞书 IM + Cron 调度器 |
| Phase 5 | W9-10 | SPEC-011, SPEC-012 | Hermes 探索 + 管理后台 |
| Phase 6 | W11-12 | SPEC-013 | 测试 |
...
| P6-03 | E2E 测试（Playwright: 编写登录→Chat→Agent→结果真实用例，替换占位用例） | 8h | P6-02 |
| P6-04 | golangci-lint 全量检查 + SonarQube 扫描修复 | 6h | All |
