...
│                     Data / Storage Layer                          │
│  ┌──────────┐  ┌───────────┐  ┌──────────┐  ┌───────────────┐  │
│  │ MongoDB  │  │  Qdrant   │  │  SeaweedFS   │  │    Redis      │  │
...
| **向量数据库** | Qdrant | 2.4+ | 高性能向量检索，CNCF 项目 |
...
│  Repository (MongoDB) | Vector Store (Qdrant)│
...
> **MongoDB 作为业务主存储**: 所有业务实体数据统一存储在 MongoDB 中。SeaweedFS 仅存储二进制大文件（Session 临时文件、Artifact 文件内容），Qdrant 仅存储向量索引，Redis 仅做缓存和消息队列。
...
上传文档 → 保存文件 → 创建索引 Agent 任务(异步)
              ...
              每个 Chunk → LLM 生成 Embedding 向量
                          ↓
              写入 kb_chunks (doc_id 强绑定) + Qdrant
                          ↓
...
func (s *KnowledgeService) indexDocument(docID string) {
    ...
    for i, chunk := range chunks {
        ...
        s.qdrant.Insert("knowledge_chunks", QdrantEntity{
            Vector:     embedding,
            DocID:      docID,
            ChunkIndex: i,
            Title:      doc.Title,
...
func (s *KnowledgeService) Search(query string, userID string, opts SearchOptions) ([]SearchResult, error) {
    ...
    case "semantic":
        ...
        qdrantResults := s.qdrant.Search("knowledge_chunks", embedding, opts.TopK)
        results = append(results, s.enrichResults(qdrantResults)...)
...
| 文件 | 用途 | 包含服务 |
|------|------|---------|
| `docker-compose.prod.yml` | 正式部署 / Demo | api, frontend, mongodb, qdrant, seaweedfs, redis, vault, mem0 |
...
# ========================
# Qdrant (Standalone — no external dependencies)
# ========================
...
services:
  ...
  api:
    ...
    environment:
      ...
      # Qdrant
      QDRANT_URL: qdrant:${QDRANT_PORT:-6334}
      # Mem0
  ...
  # ========================
  # Qdrant (Standalone — 无需 etcd/minio 依赖)
  # ========================
  qdrant:
...
# {"status":"ok","mongodb":"connected","redis":"connected","qdrant":"connected","seaweedfs":"connected"}
...
| **Qdrant 内存占用高** | 部署成本超预算 | 中 | MVP 使用小维度向量(384d) + 定期清理 | 降级到 Qdrant（更轻量） |
...
| P1-02 | Docker Compose 开发环境（MongoDB/Qdrant/SeaweedFS/Redis） | 4h | - | 参考 RFC Section 13.2 |
...
| P1-08 | Qdrant 连接层（Collection 管理 + 向量搜索） | 4h | P1-02 | 参考 RFC Section 10 |
...
| P3-04 | Qdrant Collection 创建与写入（参考 RFC Section 10.1） | 4h | P3-03, P1-08 |
...
| `knowledge-search` | 知识库搜索 | 自研 (Qdrant SDK) | ✅ |
