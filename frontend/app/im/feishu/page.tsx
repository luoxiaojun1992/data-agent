...
export default function FeishuConfigPage() {
  ...
  const t = useTranslations('feishu');
    ...
    <AppLayout>
      <div className="animate-fade-in">
        <div className="mb-6 flex items-center justify-between" data-testid="feishu-header">
          <div>
            <button onClick={() => router.push('/im')} className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] mb-1">{t('backToIm')}</button>
        ...
        {loading ? (
          <div className="text-center py-12 text-[var(--text-secondary)]" data-testid="feishu-loading">{t('loading')}</div>
        ) : configs.length === 0 ? (
          <div className="glass p-12 text-center" data-testid="feishu-empty">
            <span className="text-5xl block mb-4">{'\ud83d\udc26'}</span>
            <p className="text-lg text-[var(--text-primary)] mb-2">{t('noConfig')}</p>
            <p className="text-sm text-[var(--text-secondary)]">{t('noConfigHint')}</p>
          </div>
        ) : (
              ...
              <div key={c.id} className="glass p-4" data-testid={'feishu-row-' + c.id}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer" title={c.enabled ? t('clickDisable') : t('clickEnable')}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" data-testid="feishu-create-modal">
          ...
          <div className="relative max-w-lg w-full mx-4" style={{ ...modalPanelStyle, maxHeight: '85vh', overflowY: 'auto' }}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-[var(--text-primary)]">{t('createModalTitle')}</h3>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="text" value={newCfg.name} onChange={e => setNewCfg(p => ({ ...p, name: e.target.value }))}
                  ...
                  data-testid="feishu-name-input" placeholder={t('namePlaceholder')} />
              </div>
              <div>
                <label className="block text-xs text-[var(--text-secondary)] mb-1">{t('appId')}</label>
              ...
              <div>
                <label className="block text-xs text-[var(--text-secondary)] mb-1">{t('appSecret')}</label>
              ...
              <div>
                <label className="block text-xs text-[var(--text-secondary)] mb-1">{t('model')}</label>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={createConfig}
                  ...
                  data-testid="feishu-create-btn" disabled={!newCfg.name.trim() || !newCfg.app_id.trim() || !newCfg.app_secret.trim()}>{t('createConfig')}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {editingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" data-testid="feishu-edit-modal">
          ...
          <div className="relative max-w-lg w-full mx-4" style={{ ...modalPanelStyle, maxHeight: '85vh', overflowY: 'auto' }}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-[var(--text-primary)]">{t('editModalTitle')}</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--text-secondary)] mt-0.5">{t('modelLocked')}</p>
              </div>
              <div className="flex items-center justify-between">
                <label className="text-sm text-[var(--text-primary)]">{t('enabledStatus')}</label>
              ...
              {!editCfg.enabled && (
                <p className="text-xs text-amber-400/80">{t('disableWarning')}</p>
              )}
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={saveEdit}
                  ...
                  data-testid="feishu-save-btn">{t('save')}</button>
              </div>
            </div>
          </div>
        </div>
